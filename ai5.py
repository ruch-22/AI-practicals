...
def a_star_search(start, goal):
    ...
    # Store the g(n), f(n), and h(n) values for each step
    distance_table = []
    
    while open_set:
        _, current = heapq.heappop(open_set)
        
        # Capture the scores for the current node
        h_n = euclidean_heuristic(current, goal)
        g_n = g_score[current]
        f_n = g_n + h_n
        ...
        if current == goal:
            ...
            path.append(start)
            return path[::-1], g_score[goal], distance_table
    ...
    return None, float("inf"), distance_table

# Example usage
start_city = "Delhi"
goal_city = "Chennai"
path, distance, table_data = a_star_search(start_city, goal_city)

# Create a DataFrame for better display
df = pd.DataFrame(table_data)

print(f"Distance Table for A* Search from {start_city} to {goal_city}:")
print(df)
print("\n" + "="*50 + "\n")
